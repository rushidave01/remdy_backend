...
let pgSQl_dataSource: DataSource;
...
const connectDB = async (options?: { journeyId?: string }) => {
  ...
  try {
    ...
    logger.info(
      `[${journeyId || "Unknown"}] Attempting to connect to pgSql...`
    );
    ...
    logger.error(
      `[${journeyId || databaseConfig.database}] pgSQL connection error: ${
...
export { connectDB, getDataSource, AppDataSource, pgSQl_dataSource};
