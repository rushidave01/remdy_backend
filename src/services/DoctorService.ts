...
async getDoctorsByLocationAndRole(
  ...
  async getDoctorsByPostalCodeAndRole(
    getDoctorsDto: GetDoctorsByPostalCodeDto
  ): Promise<any> {
    const { area, building, postal_code, page, size, user_role } =
      getDoctorsDto;
  ...
  async getReviewsForDoctorsById(
    id: number,
    orderByDesc: boolean
  ): Promise<any> {
    try {
  ...
  async getDoctorDetailsById(id: number): Promise<DoctorDetails | null> {
    return await DoctorDetails.findOne({ where: { id: id } });
  }
